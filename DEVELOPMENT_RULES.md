...
- **【重要】ユーザーが新しい会話で即座に貼り付けて指示を出せるよう、作成した `handover_summary.md` の絶対パスを含めた、以下の「コピペ用指示テンプレート」をコードブロック形式でチャットの最後に必ず出力してください。**

  ```text
  作業前にルートにある DEVELOPMENT_RULES.md を確認してください。
  下記のファイルの内容を読み込んで、指示に従って進めてください。
  file:///C:/Users/toshi/.gemini/antigravity/brain/<現在の会話ID>/handover_summary.md
  ```
