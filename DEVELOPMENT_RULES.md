...
1. **実装プランの作成と承認 (Planning & Approval):**
   ...
   - **対話・質問が含まれるフィードバック受信時の作業保留と完全合意ルール (Strict Clarification Before Execution):**
     - ユーザーから複数のコメントや指示が提示された際、一部に作業の承認（例: 「統一してください」等）が含まれていても、質問・疑問・確認事項などの対話的コメント（例: 「〜の違いは何ですか？」等）が1つでも含まれている場合は、絶対に勝手に作業（コード修正・ファイル変更・コマンド実行等）を開始してはなりません。
     - 必ず最初にチャット上で質問に対する明確な回答をお返しし、ユーザーの疑問を完全に解消した上で、すべての項目についてユーザーからの完全な合意・再指示が得られてから初めて作業・実装を開始してください。
2. **ローカル実装・コンパイル検証・自律Gitコミット (Local Coding, Type Check & Auto Git Commit):**
   - 承認を得たプランに基づきローカルで実装し、完了後に必ず `npx tsc --noEmit` を実行して、TypeScriptのコンパイルエラーが「0件」であることを確認します。
   - **作業完了時の自律的Gitコミット**: 型チェックおよび検証がクリアされた後は、ユーザーからの個別指示を待つことなく、AIが自律的に適切なコミットメッセージ（Conventional Commits形式または明確な日本語）を作成し、Gitへのステージング（`git add`）およびコミット（`git commit`）を一気通貫で完了させてください。
3. **ステージング版への自動配信 (Automatic EAS Update to staging):**
   - コードの修正・変更、および型チェック（`npx tsc --noEmit`）が「0件」で成功した後は、ユーザーからの個別指示を待つことなく、AIが自律的に `otaUpdateConfig.ts` の設定内容を更新した上で、速やかに **`staging` ブランチ（ステージング用チャンネル）** へのEAS Update配信を完了させてください。
   - 実行コマンド:
     ```bash
     npx eas update -p android --branch staging --message "<更新内容の簡潔な説明>"
     ```
   
   > [!IMPORTANT]
   > **修正完了からステージングOTA配信までの一気通貫ルール:**
   > コード修正・バグ対策の承認（Goサイン）を得て実装した後は、単にコードを書き換えて型チェックを行うだけでなく、ステージング環境（`staging`）へのEAS Update配信までを「1つの不可分なタスク」として完結させてください。配信完了後に、ユーザー様へ実機テストの依頼とUpdate IDの提示を行ってください。
...
10. **OTA連続配信上限とネイティブビルド推奨通知ルール (OTA Limits & Build Suggestion Rule):**
    - **キャッシュ混線防止のアラート基準**: 同一ネイティブビルド（`versionCode` / `runtimeVersion`）に対する OTA アップデート配信回数が **30〜40 回に達した場合**、AIは自発的にユーザーに対して「OTA配信が 30〜40 回を超えました。端末上のキャッシュ混線やアップデート確認エラーを未然に防止するため、新しい AAB/APK ネイティブビルドの再生成（`versionCode` インクリメント）をご検討ください」と提案・注意喚起を行わなければなりません。
...
- **AI Coach チャット (`/api/chat`) ＆ 画像栄養解析 (`/api/nutrition-image`)**:
  - 使用する公式プライマリモデルは **`gemini-3.7-flash`** です。最優先モデルとして常に指定・維持してください。
  - 429 (Quota制限) や API エラーが発生した場合は、Worker 内で以下の順序で自動リトライ・即時切替を行う多重冗長化仕様となっています：
    1. **`gemini-3.7-flash`** (プライマリ / 10,000 RPD / 最優先)
    2. **`gemini-3.6-flash`** (セカンダリ / 10,000 RPD)
    3. **`gemini-3.5-flash`** (サード / 10,000 RPD)
    4. **`gemini-3.5-flash-lite`** (クォータリ / 安定版 Lite)
    5. **`gemini-3.1-flash-lite`** (フォールバック1 / 高速版 Lite)
    6. **`gemini-flash-lite-latest`** (最終フォールバック / 最新Lite)
    7. **`deepseek-v4-pro`** (DeepSeek API / 緊急フォールバック)

- **テキスト食事・栄養解析 (`/api/nutrition`)**:
  - 高速応答・低リソース消費に最適化された軽量 Lite モデルの多重構成を採用：
    1. **`gemini-3.5-flash-lite`** (プライマリ / 検証済みの安定版)
    2. **`gemini-3.1-flash-lite`** (セカンダリ / 高互換な高速版)
    3. **`gemini-flash-lite-latest`** (セーフティネット / 最新Lite)
    4. **`deepseek-v4-pro`** (DeepSeek API / 最終フォールバック)
